...
public static class LondonUnderground
{
    ...
    public static Dictionary<string, Station> Stations { get; set; }
    //{Line name : Connections on that line}
    public static Dictionary<string, Dictionary<string ,List<Connection>>> Connections { get; set; }

    public static TextWriter writer = new TextWriter();
    public static void Start()
    {
        Stations = new Dictionary<string, Station>();
        Connections = new Dictionary<string, Dictionary<string, List<Connection>>>();
        LoadDataFromCSV("Inter Station Train Times.csv");
    ...
    private static (List<(Station, string)>, int) ShortestPath(string startName, string endName)
    {
        ...
        {
        Console.Clear();
        File.Delete("V2_output.txt");
        Console.WriteLine($"Route: {shortestPath.Item1.First().Item1.Name} to {shortestPath.Item1.Last().Item1.Name}:");
        writer.WriteToFile($"Route: {shortestPath.Item1.First().Item1.Name} to {shortestPath.Item1.Last().Item1.Name}:");
        ...
        foreach (var stationTuple in shortestPath.Item1)
        {
            ...
            if (station != prev)
            {
                if (station.Previous != null)
                {
                    ...
                    writer.WriteToFile("(" + (shortestPath.Item1.IndexOf(stationTuple)+1) + ") " + station.Name + " (Line: " + line +")" + " " + (station.TimeFromStart - station.Previous.TimeFromStart));   
                }
                else
                {
                    Console.WriteLine("(" + (shortestPath.Item1.IndexOf(stationTuple)+1) + ") " + station.Name + " (Line: " + line +")");
                    writer.WriteToFile("(" + (shortestPath.Item1.IndexOf(stationTuple)+1) + ") " + station.Name + " (Line: " + line +")");   
                }
                prev = station;
            }
            else
        ...
        writer.WriteToFile($"Total Journey Time: {shortestPath.Item1.Last().Item1.TimeFromStart} minutes");
